...
type
  TForm_Estatistica_Hospedagem = class(TForm)
    ...
    ClientDataSet_Hospedesaram: TStringField;
    ClientDataSet_Hospedeficha: TStringField;
    ...
    ClientDataSet_Hospedetipo: TStringField;
    ClientDataSet_Hospedediarias: TFloatField;
    ClientDataSet_Hospedetempo: TFloatField;
    ClientDataSet_Hospededep: TFloatField;
    ClientDataSet_Hospedetotal: TFloatField;
    DBGrid_Ativ: TDBGrid;
    DSC_Hospede: TDataSource;
    ...
    procedure JvDateEdit_InicioClick(Sender: TObject);
    procedure JvDateEdit_DataTerminoClick(Sender: TObject);
...
var
  Form_Estatistica_Hospedagem: TForm_Estatistica_Hospedagem;
...
procedure TForm_Estatistica_Hospedagem.FormActivate(Sender: TObject);
begin
...
procedure TForm_Estatistica_Hospedagem.ToolButton9Click(Sender: TObject);
begin
   Close;
end;

procedure TForm_Estatistica_Hospedagem.ToolButton_AtividadesClick(Sender: TObject);
var
   Caracter, VS_Posto, VS_Comando, VS_Data_Entrada,VS_Tipo, VS_Dep, VS_Duplicidade: String;
   ...
   Total_Fechamentos, Total_Cancelamentos, Total_Hospedes, Total_Registros: Real;
...
begin
   ...
   begin
    ...
    //     Memo1+ .Lines.Clear;
         ...
         VS_Comando:= 'Select a.ficha, b.Nome, b.posto, a.dataentrada, a.datasaida, a.Tipo_Fechamento, a.ValorPago, a.obs, a.horaSaida, a.ValorPago '+
                      ' From Historico a, hospede b '+
                      ' where a.identidade = b.identidade '+
                      ' and  a.TIPO_FECHAMENTO <> '+#39+'C'+#39+
...
//         Memo1.Lines.Add(VS_Comando+' order by b.posto, a.dataentrada, b.Nome ');
         ...
         begin
          //--- Classificação por situacao de
            ...
            begin
               ...
               ClientDataSet_Hospede.FieldByName('saram').AsString:='';
               ClientDataSet_Hospede.FieldByName('ficha').AsString:= Query_Atividades.FieldByName('ficha').AsString;
               ...
               ClientDataSet_Hospede.FieldByName('Entrada').AsString:= Query_Atividades.FieldByName('dataentrada').AsString;
               ClientDataSet_Hospede.FieldByName('Saida').AsString:= Query_Atividades.FieldByName('datasaida').AsString;
               ClientDataSet_Hospede.FieldByName('tipo').AsString:= VS_Tipo;
               ClientDataSet_Hospede.FieldByName('Diarias').AsString:= FormatFloat('####0.00', Query_Atividades.FieldByName('ValorPago').AsFloat);
               ClientDataSet_Hospede.FieldByName('tempo').AsString:= FormatFloat('####0.00', Tempo);
               ClientDataSet_Hospede.FieldByName('dep').AsString:= FormatFloat('####0.00', Dependente);
               ClientDataSet_Hospede.FieldByName('total').AsString:= FormatFloat('####0.00', Soma);

               ClientDataSet_Hospede.Post;


               Memo2.Lines.Add( FormatFloat('000', Tempo)+'   - '+
                                VS_Dep+' - '+
   ...
   Labe_Total_Reg.Caption:= FormatFloat('####0.0', Total_Hospedes + Total_Fechamentos + Total_Cancelamentos);
...
procedure TForm_Estatistica_Hospedagem.ToolButton10Click(Sender: TObject);
begin
   Funcoes.Windows;
end;

procedure TForm_Estatistica_Hospedagem.SpeedButton_Atualiza_MapaClick(Sender: TObject);
begin
   Calcular;
end;

procedure TForm_Estatistica_Hospedagem.Calcular;
var
...
procedure TForm_Estatistica_Hospedagem.ToolButton_VisualicaoaClick( Sender: TObject);
begin
...
procedure TForm_Estatistica_Hospedagem.Grava(Cod: integer; Desc: String; Valor: Real);
begin
...
procedure TForm_Estatistica_Hospedagem.JvDateEdit_InicioClick(Sender: TObject);
begin
   if JvDateEdit_DataTermino.Text = '  /  /    ' then
       JvDateEdit_DataTermino.Date:= JvDateEdit_Inicio.Date;
end;
procedure TForm_Estatistica_Hospedagem.JvDateEdit_DataTerminoClick(Sender: TObject);
begin
   if JvDateEdit_Inicio.Text = '  /  /    ' then
       JvDateEdit_Inicio.Date:= JvDateEdit_DataTermino.Date;
end;
procedure TForm_Estatistica_Hospedagem.DBGrid_AtivDrawColumnCell(
...
//   begin
      ...
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'Oficiais_Generais' then
         DBGrid_Ativ.Canvas.Brush.Color:= $006BB6B6
      else
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'Oficiais Intermediarios' then
         DBGrid_Ativ.Canvas.Brush.Color:= $00D2E1FF
      else
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'Oficiais Subalternos Aspirantes' then
         DBGrid_Ativ.Canvas.Brush.Color:= $00ACD7D7
      else
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'SO SGT' then
         DBGrid_Ativ.Canvas.Brush.Color:= $00FF8CFF
      else
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'CB SD TF' then
         DBGrid_Ativ.Canvas.Brush.Color:= $009B9B00
      else
      if trim(ClientDataSet_Hospede.FieldByName('tipo').AsString) = 'Civil' then
...
procedure TForm_Estatistica_Hospedagem.ToolButton_ExcelClick(Sender: TObject);
var
...
//      Connect := not (VarIsEmpty(OpenOffice) or VarIsNull(OpenOffice));
      ...
      i:= i + 1;
      Sheet.getCellByPosition(0,i).SetString('FICHA');
      Sheet.getCellByPosition(1,i).SetString('NOME');
      Sheet.getCellByPosition(2,i).SetString('POSTO');
      Sheet.getCellByPosition(3,i).SetString('DATA ENTRADA');
      Sheet.getCellByPosition(4,i).SetString('DATA SAÍDA');
      Sheet.getCellByPosition(5,i).SetString('TIPO');
      Sheet.getCellByPosition(6,i).SetString('DIÁRIA');
      Sheet.getCellByPosition(7,i).SetString('TEMPO');
      Sheet.getCellByPosition(8,i).SetString('DEP.');
      Sheet.getCellByPosition(9,i).SetString('TOTAL');
      ...
      begin
         ...
         i:= i+1;
         Sheet.getCellByPosition(0,i).SetString(ClientDataSet_Hospede.FieldByName('Ficha').AsString);
         Sheet.getCellByPosition(1,i).SetString(ClientDataSet_Hospede.FieldByName('Nome').AsString);
         Sheet.getCellByPosition(2,i).SetString(ClientDataSet_Hospede.FieldByName('posto').AsString);
         Sheet.getCellByPosition(3,i).SetString(ClientDataSet_Hospede.FieldByName('entrada').AsString);
         Sheet.getCellByPosition(4,i).SetString(ClientDataSet_Hospede.FieldByName('saida').AsString);
         Sheet.getCellByPosition(5,i).SetString(ClientDataSet_Hospede.FieldByName('tipo').AsString);
         Sheet.getCellByPosition(6,i).SetString(ClientDataSet_Hospede.FieldByName('Diarias').AsString);
         Sheet.getCellByPosition(7,i).SetString(ClientDataSet_Hospede.FieldByName('tempo').AsString);
         Sheet.getCellByPosition(8,i).SetString(ClientDataSet_Hospede.FieldByName('dep').AsString);
         Sheet.getCellByPosition(9,i).SetString(ClientDataSet_Hospede.FieldByName('total').AsString);
