...
type
  TForm_MostraTabPreco = class(TForm)
    ...
    Query_Tabela_PrecoPOSTOGRADUAO: TStringField;
    Query_Tabela_PrecoINICIO: TFloatField;
    Query_Tabela_PrecoTERMINO: TFloatField;
...
begin
   ...
   MASKEDIT_Posto.Text:= '';
   VS_OrderBy:= 'Predio, PostoGraduação, inicio';
   Busca_Tabela(Sender);
end;
...
begin
   ...
   begin
      if VS_Where = '' then
         VS_Comando:= VS_Comando +' Where postoGraduação like '+#39+MASKEDIT_Posto.text+'%'+#39+' '
      else
         VS_Comando:= VS_Comando +'and postoGraduação like '+#39+MASKEDIT_Posto.text+'%'+#39+' ';
   end;
