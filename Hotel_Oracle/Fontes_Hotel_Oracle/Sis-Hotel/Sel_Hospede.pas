...
uses
  ...
  StdCtrls, Db, DBTables, Grids, DBGrids, ComCtrls, Buttons, ExtCtrls,
  ToolWin;
...
begin
   ...
   Query_Sel_Hospede.Sql.Add(VS_Comando+VS_OrderBy);
...
begin
   if Query_Sel_Hospede.RecordCount > 0 then
   begin
      Funcoes.SetIdentidade (Query_Sel_Hospede.FieldByName('Identidade').AsString);
   end
   else
   begin
      if Trim(Edit_Identidade.Text) = '' then
