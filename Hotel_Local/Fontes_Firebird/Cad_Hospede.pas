...
uses
  ...
  JvLabel, JvBlinkingLabel, JvGradient, DBTables, JvComponent, JvEnterTab;
...
type
  TForm_Cad_Hospede = class(TForm)
    ...
    DSQ_Busca_PostoGraduacao: TDataSource;
    ...
    IBQuery_Busca_PostoGraduacao: TIBQuery;
    ...
    SpeedButton_MostraFicha: TToolButton;
    SpeedButton_AmpliaMapa: TToolButton;
    ...
    IBTable_CadHospedeSITUACAO: TIBStringField;
    IBTable_CadHospedeOBS: TIBStringField;
    IBTable_CadHospedeSEXO: TIBStringField;
    IBTable_CadHospedeTELEFONE_CELULAR: TIBStringField;
    IBTable_CadHospedeEMAIL: TIBStringField;
    IBTable_CadHospedeVEICULO: TIBStringField;
    IBTable_CadHospedePLACA: TIBStringField;
    IBTable_CadHospedeCPF: TIBStringField;
    ...
    procedure Monta_Posto_Predio(Sender: TObject);
   ...
   procedure MaskEdit_FichaKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure MaskEdit_IdentidadeKeyPress(Sender: TObject; var Key: Char);
  private
...
begin
   ...
   begin
      ...
      IBQuery_Curso.Open;
      IBQuery_Curso.First;
      While not IBQuery_Curso.eof do
      begin
         ComboBox_Situacao_Curso.Items.Add(IBQuery_Curso.FieldByName('Curso').AsString);
         IBQuery_Curso.Next;
      end;
      IBQuery_Curso.Close;
...
begin
   ...
   MaskEdit_Ficha.Text:= Funcoes.NFicha(IntToStr(VI_Ficha+1));
...
begin
   ...
   begin
      ...
      begin
          VS_Comando:= 'Select PostoGraduacao from RegrasDeDiarias where PostoGraduacao = '+#39+trim(ComboBox_Posto_Graduacao.Text)+#39;
      ...
      begin
         if ((trim(ComboBox_Dependente_de.Text) <> '') and (trim(ComboBox_Posto_Graduacao.Text) = '') and (VB_Grava = True))  then
         begin
            VS_Comando:= 'Select * from RegrasDeDiarias where PostoGraduacao = '+#39+trim(ComboBox_Dependente_de.Text)+#39;
            if Funcoes.Procura(VS_Comando) = False then
            begin
      ...
      begin
         ...
         begin
            VS_Comando:= 'Select PostoGraduacao from RegrasDeDiarias where PostoGraduacao = '+#39+trim(ComboBox_Posto_Graduacao.Text)+#39+
                         ' and  predio = '+#39+trim(Edit_Predio.Text)+#39;
         end
         else
         begin
            VS_Comando:= 'Select * from RegrasDeDiarias where PostoGraduacao = '+#39+trim(ComboBox_Dependente_de.Text)+#39+
      ...
      if ((trim(ComboBox_Posto_Graduacao.Text) <> '') and (VB_Grava = True))  then
      begin
         VS_Comando:= 'Select PostoGraduacao from RegrasDeDiarias '+
                      ' where PostoGraduacao = '+#39+trim(ComboBox_Posto_Graduacao.Text)+#39+
                      ' and predio = '+#39+trim(Edit_Predio.Text)+#39;
         ...
         begin
            MessageDlg('O Posto/Graduação de '+trim(ComboBox_Posto_Graduacao.Text)+' não ser hospedado no '+
                        PredioAndar+trim(IBTable_CadHospedagem.FieldByName('Predio').AsString)+' .'+#13+
      ...
      begin
         ...
         IBTable_CadHospede.FieldByName('CPF').AsString:= MaskEdit_CPF.Text;
         IBTable_CadHospede.FieldByName('TELEFONE_CELULAR').AsString:=  MaskEdit_Telefone_Celular.Text;
         IBTable_CadHospede.FieldByName('EMAIL').AsString:= MaskEdit_EMail.Text;
         ...
         ToolBar_Rodape.Enabled:= True;

         TrataBotoes_Hospede;
         TrataCampos(True);
         bmLocal:= DM.IBQuery_Mapa.GetBookMark;
         Monta_Mapa(1);
         DM.IBQuery_Mapa.GotoBookMark(bmLocal);
         DM.IBQuery_Mapa.FreeBookMark(bmLocal);
      end;
   end;
end;
...
begin
   IBQuery_Sel_Dep.Close;
   IBQuery_Sel_Dep.Sql.Clear;
   IBQuery_Sel_Dep.Sql.Add('Select * from dependente where ficha = '+#39+Funcoes.NFicha(MaskEdit_Ficha.Text)+#39);
   IBQuery_Sel_Dep.Open;
...
begin
    ...
    begin
       ...
       MaskEdit_Telefone_Celular.Text:= IBTable_CadHospede.FieldByName('TELEFONE_CELULAR').AsString;
...
begin
   ...
   begin
      ...
      begin
         MessageDlg('O hóspede '+IBQuery_Busca_Hospede.FieldByName('nome').AsString+' possuí restrição, não pode ser hospedado,'+#13+#13+
      ...
      begin
         ...
         begin
            ...
            MaskEdit_Telefone_Celular.Text:= IBQuery_Busca_Hospede.FieldByName('TELEFONE_CELULAR').AsString;
...
procedure TForm_Cad_Hospede.Monta_Posto_Predio(Sender: TObject);
var
   VS_Comando: String;
begin
    VS_Comando:= 'Select distinct PostoGraduacao from RegrasDeDiarias where predio = '+#39+Edit_Predio.Text+#39;
    IBQuery_Busca_PostoGraduacao.Close;
    IBQuery_Busca_PostoGraduacao.Sql.Clear;
    IBQuery_Busca_PostoGraduacao.Sql.Add(VS_Comando);
    IBQuery_Busca_PostoGraduacao.Open;
    ...
    IBQuery_Busca_PostoGraduacao.First;
    While not IBQuery_Busca_PostoGraduacao.Eof do
    begin
       ComboBox_Posto_Graduacao.Items.Add(IBQuery_Busca_PostoGraduacao.FieldByName('PostoGraduacao').AsString);
       ComboBox_Dependente_de.Items.Add(IBQuery_Busca_PostoGraduacao.FieldByName('PostoGraduacao').AsString);
       IBQuery_Busca_PostoGraduacao.Next;
    end;
end;
...
procedure TForm_Cad_Hospede.MaskEdit_IdentidadeKeyPress(Sender: TObject;
  var Key: Char);
begin
   if not (Key in ['0'..'9' , ',', 'D', 'd' , #8]) then Key := #0;
end;
