...
uses Module, Rel_Rela_Reserva, Menu_Principal;
      ...
      begin     //--- Tratamento para verificar se foi fornecido o numero da identidade
         ...
         begin
            ...
            begin
               ...
               if ((trim(ComboBox_Posto_Graduacao.Text) <> '') and (VB_Gravar = True))  then
               begin
                  VS_Comando:= 'Select * from RegrasDeDiarias where PostoGraduacao = '+#39+trim(ComboBox_Posto_Graduacao.Text)+#39+
                               ' and  predio = '+#39+trim(IBQuery_Visao.FieldByName('Predio').AsString)+#39;
...
begin
   if ComboBox_Predio.Text = '' then
      ShowMessage('Para efetuar a Reserva é necessário selecionar o Prédio e a cama, clicando no quadriculo da data de entrada..');
...
var
   VS_Comando: String;
begin
    VS_Comando:= 'Select distinct PostoGraduacao from RegrasDeDiarias where predio = '+#39+IBQuery_Visao.FieldByName('Predio').AsString+#39;
...
begin
   Form_Menu_Pri_G.IBQuery_Reserva.Open;
...
procedure TForm_MontaReserva.MaskEdit_IdentidadeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TForm_MontaReserva.ComboBox_PredioExit(Sender: TObject);
begin
   if ComboBox_Predio.Text = '' then
   begin
       ShowMessage('Para realizar a reserva é necessário selecionar o prédio...');
       ComboBox_Predio.SetFocus;
   end;
end;
