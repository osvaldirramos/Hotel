...
type
  TForm_Cad_Reserva = class(TForm)
    ...
    Panel1: TPanel;
    DBNavigator_Reserva: TDBNavigator;
    ToolBar4: TToolBar;
    ...
    IBTable_ReservaSEXO: TIBStringField;
    Label3: TLabel;
...
begin
   ...
   begin
       VS_Comando:= 'Select count(1) as Qtd from TABELA_POSTO where posto = '+#39+trim(DBComboBox_Posto_Graduacao.Text)+#39;
   ...
   begin
      if ((trim(DBComboBox_Dependente_de.Text) <> '') and (trim(DBComboBox_Posto_Graduacao.Text) = '') and (VB_Grava = True))  then
      begin
         VS_Comando:= 'Select count(1) as Qtd  from TABELA_POSTO where posto = '+#39+trim(DBComboBox_Dependente_de.Text)+#39;
         if Funcoes.Procura(VS_Comando) = False then
         begin
...
procedure TForm_Cad_Reserva.ToolButton_IgnorarClick(Sender: TObject);
begin
   Try
...
begin
   DBNavigator_Reserva.Enabled:= Status;
