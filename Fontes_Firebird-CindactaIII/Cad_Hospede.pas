   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
               VS_Comando:= 'Select count(1) as Qtd from hospedagem where Ficha = '+#39+trim(MaskEdit_Ficha.Text)+#39+' and ano = '+Funcoes.AnoAtual;
               ...
               begin
                  VS_Comando:= 'Select count(1) as Qtd from historico where Ficha = '+#39+trim(MaskEdit_Ficha.Text)+#39+' and ano = '+Funcoes.AnoAtual;
                  if Funcoes.Procura(VS_Comando) = true then
                  begin
                     MessageDlg('Já existe hospedagem com a Ficha ('+MaskEdit_Ficha.Text+').', mtInformation,[mbOk], 0);
                     VB_Grava:= False;
      ...
      begin
         if ((trim(ComboBox_Dependente_de.Text) <> '') and (trim(ComboBox_Posto_Graduacao.Text) = '') and (VB_Grava = True))  then
         begin
            VS_Comando:= 'Select count(1) as Qtd from TABELA_POSTO where posto = '+#39+trim(ComboBox_Dependente_de.Text)+#39;
            if Funcoes.Procura(VS_Comando) = False then
            begin
...
begin
    ...
    begin
       ...
       ComboBox_Sexo.Text:= IBTable_CadHospede.FieldByName('SEXO').AsString;
...
begin
   ...
   begin
      VS_Comando:= 'Select count(1) as Qtd from Tabela_Decea where Localidade = '+#39+trim(ComboBox_OM_Origem.Text)+#39;
      if Funcoes.Procura(VS_Comando) = True then
         RadioGroup_Desconto.ItemIndex:= 0;
   end;
end;
