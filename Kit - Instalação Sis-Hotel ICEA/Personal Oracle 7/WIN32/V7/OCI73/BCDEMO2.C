#ifdef RCSID
static char *RCSid = 
   "$Header: cdemo2.c 7020100.1 94/09/23 22:19:31 cli Generic<base> $ ";
#endif /* RCSID */
...
/*
   ...
   MODIFIED   (MM/DD/YY)
    emendez    06/28/94 -  fix ansi warnings
    emendez    04/07/94 -  merge from branch 1.8.710.3
    emendez    04/07/94 -  merge changes from branch 1.8.710.2
    gdoherty   04/06/94 -  merge changes from branch 1.8.710.1
...
connect_user()
{
    ...
    for (n = 3; --n >= 0; )
    {
        ...
        if (olog(&lda, hda, username, -1, password, -1, (text *)0,-1,(ub4)OCI_LM_DEF))
        {
